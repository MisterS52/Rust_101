fn main() {
    ...
    // Mal Esto daría error de compilación porque Rust no convierte automáticamente entre tipos
    // let suma = entero + flotante; 
    // En C sí dejaría, pero en Rust hay que ser explícito

    // Bien Forma correcta: convertir manualmente el entero a flotante
    let suma = entero as f64 + flotante;
    println!("Suma correcta (con conversión): {}", suma);

    // Mal Esto daría error porque no se puede usar un booleano como número
    // let resultado = entero + booleano; 

    // Bien Si realmente queremos usarlo, tenemos que hacer la conversión explícita
    ...
    // Bien Acceso seguro a un elemento de un arreglo
    println!("El primer elemento del arreglo es: {}", array[0]);

    // Mal Si intentamos acceder fuera del rango, Rust detecta el error en tiempo de ejecución
